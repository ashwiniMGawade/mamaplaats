...
class Migration_Core_20140520163732 extends Minion_Migration_Base {
	...
	{
		$products = $db->query(Database::SELECT, "SELECT * FROM review_products");
		foreach ($products as $product)
		{
			// Only migrate stuff without sections
			$sections = $db->query(Database::SELECT, "SELECT * FROM review_product_sections WHERE review_product_id = ".$product['id']);
			if (count($sections) >= 1)
			{
				continue;
			}
