...
function template_functions(){
	...
	$('a[data-confirm]').click(function(ev) {
		...
		if (!$('#dataConfirmModal').length) {
			$('body').append('<div id="dataConfirmModal" class="modal" role="dialog" aria-labelledby="dataConfirmLabel" aria-hidden="true"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button><h3 id="dataConfirmLabel">Gelieve te Bevestigen</h3></div><div class="modal-body"></div><div class="modal-footer"><button class="btn" data-dismiss="modal" aria-hidden="true">Cancel</button><a class="btn btn-primary" id="dataConfirmOK">OK</a></div></div>');
		}
	...
	/* ---------- Dynamic Rows (Table - sidebanners) ---------- */
	$("#sidebanners .add_dynamic_row").click(function(el){
		el.preventDefault();
		var row = $('#banner_ad_placeholder').html();
		var new_row = $('#sidebanners tbody').append('<tr class="instance_dynamic_row"><td>'+row+'</td></tr>').show().css('visibility','visible');
		$('input:file', new_row).uniform();
	});

	$("#sidebanners .delete_dynamic_row").live('click',function(el){
		el.preventDefault();
		$(this).closest('.instance_dynamic_row').remove();
	});

	/* ---------- Dynamic Rows (Table - submenu) ---------- */
	var counter = parseInt($('#submenu_counter').text())-1;
	var subcounter = 0;
	$("#submenu .btn.add_dynamic_row").click(function(el){
		el.preventDefault();
		counter++;
		var row = $('#submenu_placeholder tr').last().clone();
		$('#submenu tbody').append(row);
		$('#sub_header', row).attr('name', 'sub_header['+counter+'][]');
		$('#sub_title', row).attr('name', 'sub_title['+counter+'][]');
		$('#sub_url', row).attr('name', 'sub_url['+counter+'][]');
	});

	$("#submenu .add_dynamic_subrow").live('click',function(el){
		el.preventDefault();
		var row = $('#submenu_placeholder .submenu_item .submenu_row').clone();

		$(this).before(row);
		subcounter = parseInt($('.submenu_rowid', $(this).parent().parent()).text());
		if(isNaN(subcounter)) subcounter = counter;
		$('#sub_title', row).attr('name', 'sub_title['+subcounter+'][]');
		$('#sub_url', row).attr('name', 'sub_url['+subcounter+'][]');
	});

	$("#submenu .delete_dynamic_submenu").live('click',function(el){
		el.preventDefault();
		$(this).closest('tr').remove();
	});

	$("#submenu .delete_dynamic_link").live('click',function(el){
		el.preventDefault();
	...
	/* ---------- Text editor ---------- */
	$('.redactor_content, textarea.tinymce').redactor({
		lang: 'nl',
		minHeight: 200,
...
function charts() {
	...
	if($("#stats-chart2").length)
	{
		...
		$.ajax({
			async: false,
			url: 'dashboard/ga_flot_chart',
			dataType:"json",
			success: function(data) {
				a4y = data;

				$('#flot-date-range').html(a4y.date_range);

				var i = 1;
				$.each( a4y.total, function( key, value ) {
					$('#flot-range'+i+' .number').html(value);
					i++;
				});

				var i = 1;
				$.each( a4y.percentage, function( key, value ) {
					$('#flot-range'+i+' .percent').html(value+'%');
					if(value < 0){
						$('#flot-range'+i+' .arrow img').attr("src","assets/img/downarrow.png");
					}
					i++;
				});

				var i = 1;
				$.each( a4y.flot_data, function( key, value ) {
					$('#flot-range'+i+' .text').html(value.label);
					i++;
				});
			}
		});

		var plot = $.plot($("#stats-chart2"), a4y.flot_data, {
				series: {
					lines: {
						show: true,
						lineWidth: 2
				...
				xaxis: {ticks:15, tickDecimals: 0,mode:'time'},
...
function partnerPage(){
	if($('#category option[data-partner]').length){

		$('ul.nav-tabs li').bind('click', function (e) {
			var selected = $('#partner').val();
			$('select#category option[data-partner]').hide();
			$('select#category').children().filter(function(){
				return $(this).data('partner') === selected;
			}).show();
		});
